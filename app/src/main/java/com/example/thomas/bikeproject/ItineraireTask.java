...
public class ItineraireTask extends AsyncTask<Void, Integer, Boolean> {
    ...
    protected Boolean doInBackground(Void... params) {
        try {
            ...
            url.append("http://maps.googleapis.com/maps/api/directions/xml?sensor=false&language=fr");
            url.append("&origin=");
            ...
            /*
            possibles : driving, walking, bicycling, transit
             */
            url.append("&mode=bicycling&alternatives=true");
    ...
    protected void onPostExecute(final Boolean result) {
        if(!result) {
            ...
            markerB.icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_VIOLET));

            //On met à jour la carte
            gMap.moveCamera(CameraUpdateFactory.newLatLngZoom(lstLatLng.get(0), 15));
